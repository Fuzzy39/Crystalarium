    ...
    public class Timekeeper
    {
        ...
        public string CreateReport()
        {
            string toReturn = "--------TIMING REPORT----------\n" +
                ...
                "Used Time: " + Util.MiscUtil.FormatTime(UsedTime) + " (" + Math.Round((UsedTime / targetElapsed) * 100, 1) + "%):";
            ...
            toReturn += Util.MiscUtil.Indent(s);
            // this line is hideous
            return toReturn + "Other: " + ((UsedTime > AccountedTime) ? Util.MiscUtil.FormatTime(UsedTime - AccountedTime) + " (" +
                Math.Round(((UsedTime - AccountedTime) / UsedTime) * 100, 1) + "%)" : Util.MiscUtil.FormatTime(new TimeSpan()) + " (0%)")
                + "\nFree Time: " + Util.MiscUtil.FormatTime(FreeTime) + " (" + Math.Round(FreeTime / total.AverageLength * 100, 1) + "%)\n-------------------------------";
        }
